...
```bash
yarn add -D @expressots/studio@next @expressots/studio-agent@next
```
...
```bash
pnpm add -D @expressots/studio@next @expressots/studio-agent@next
```
...
Starting with v4.0.0-preview, the agent accepts a `mode` and `installId` configuration for future production telemetry and cloud features. See [Production & Telemetry](./production-telemetry.mdx) for details.
:::
