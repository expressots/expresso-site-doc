...
Starting with v4.0.0-preview, Studio Agent ships two foundational configuration fields — `mode` and `installId` — that prepare your application for production monitoring, anonymous telemetry collection, and future enterprise cloud services without requiring a breaking API change.
