...
For the In-Memory DB, `BaseRepository<T>` provides a Prisma-like CRUD interface. Inject the `InMemoryDBProvider` and pass the table name and entity class to `super()`:
...
`BaseRepository` exposes `findUnique()`, `findFirst()`, `findMany()`, `create()`, `createMany()`, `update()`, `updateMany()`, `upsert()`, `delete()`, `deleteMany()`, `count()`, `aggregate()`, and `transaction()`. See **[In-Memory DB](../features/in-memory-db.mdx)** for the full Prisma-like query API.
