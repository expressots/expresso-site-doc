...
:::info
MicroAPI projects have the entire application setup in the `api.ts` file. The `micro()` function creates a simplified API instance with auto-JSON parsing and Express-style routing.
:::
