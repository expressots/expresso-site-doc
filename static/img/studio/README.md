...
Until real screenshots are dropped here, the docs render with **branded placeholder
PNGs** generated by `_make_placeholders.sh`. They use the same dark visual language
as the rest of the site and label themselves clearly as "MISSING SCREENSHOT" so the
gap is undeniable to anyone browsing the docs.

To regenerate placeholders (e.g. after editing labels in the script):

```bash
cd static/img/studio
bash _make_placeholders.sh
```

When you drop a real PNG with the same filename, it overrides the placeholder.
