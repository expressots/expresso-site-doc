...
For all ExpressoTS releases, bug fixes and security patches are provided after the release date. After that period, the release is end-of-life unless an `Enterprise customer` or `individual` holds a valid extended-support contract.

Starting with v4.0.0, the support window is **24 months**. v3.0.0 was on the original 18-month policy and was **extended by 6 months** in May 2026 to give users a smooth upgrade runway to v4.
...
For the latest support matrix see the [v4.0.0 release notes](/docs/4.0.0/prologue/release).
