...
`@expressots/studio` is published under the npm `preview` dist-tag for the v4.0.0 line. The full release ships with **v4.1.0**. To install the preview:

```bash
npm install -D @expressots/studio@preview
```

When you launch it, the CLI prints a banner listing the known limitations.
