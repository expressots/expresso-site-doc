...
Studio ships as two preview packages. Install both as **dev dependencies**:
...
```bash
yarn add -D @expressots/studio@preview @expressots/studio-agent@preview
```
...
```bash
pnpm add -D @expressots/studio@preview @expressots/studio-agent@preview
```
...
| `@expressots/studio-agent`    | Loaded into your app process — instruments routes, records, scans |
...
If you see `npm WARN ... @expressots/adapter-express` complaining about `@expressots/studio-agent` peer dependency, ensure both `adapter-express` and `studio-agent` are on the same v4 line. Studio v4.0.x preview pairs with adapter-express v4.0.x.
