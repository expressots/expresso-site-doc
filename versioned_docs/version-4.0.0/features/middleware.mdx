...
Middleware presets are catalogued in code; you can read them and create your own without touching `this.Middleware.applyPreset(...)`.
...
const presetNames     = getPresetNames();             // ["api", "production", "development", ...]
const securityPresets = getPresetsByTag("security");  // every preset tagged "security"

const customPreset = createPreset({
  name: "edge",
  description: "Optimized for serverless edge runtimes",
  middleware: [
    { name: "cors", options: { origin: true } },
    { name: "json", options: { limit: "1mb" } },
  ],
  tags: ["serverless"],
});

const apiPlusEdge = mergePresets(apiPreset, customPreset);
```

You can then apply a custom preset via `this.Middleware.applyPreset(customPreset.name)` once registered.
