...
ExpressoTS v4 features a **zero-configuration lazy loading system** that automatically detects routes and loads modules on-demand, dramatically reducing application startup time and memory usage.

## Overview

The lazy loading system provides:

- **Zero-Config Auto-Detection**: Routes automatically detected from `@controller()` decorators
- **One-Liner Setup**: Simple `setupLazyLoadingForExpress()` function
- **Warmup Strategies**: Idle, immediate, or manual warmup
- **Preload Hints**: Low, medium, high, or never priority
- **Performance Metrics**: Built-in tracking and monitoring
- **Automatic Middleware**: Auto-load middleware for lazy routes
