...
ExpressoTS v4 features a **production-ready logging system** with 11 comprehensive phases, providing everything from basic logging to advanced features like query/export, health monitoring, flow visualization, and sensitive data redaction.
...
- **Structured Logging**: Log objects with full metadata
- **Multiple Transports**: Console, File, Custom, and more
- **Context Management**: Child loggers and context tracking
- **Data Redaction**: Automatic sensitive data protection
- **Performance Tracking**: Built-in metrics and monitoring
- **Query & Export**: In-memory buffer with search and export
- **Flow Visualization**: ASCII art event flow tracking
...
ExpressoTS provides 6 log levels with intelligent filtering:
...
Transports control where logs are written (console, files, databases, etc.).
...
Automatically redact sensitive data from logs (passwords, API keys, credit cards, etc.).
...
## Phase 7: Flow Visualization

Visualize event flow through your application:
...
## Phase 11: Query & Export

Query logs from in-memory buffer and export to various formats:
...
this.logger.configure({ level: LogLevel.DEBUG });
```

2. **Verify transports are configured:**
```typescript
this.logger.configure({
    transports: [ConsoleTransport.forDevelopment()]
});
...
3. **Check environment:**
```typescript
// Production might have higher log level
...
**Problem:** Sensitive data is not being redacted

**Solution:**
```typescript
configureGlobalRedactor({
    enabled: true,  // Ensure redaction is enabled
    customFieldPatterns: ["mySecret", "apiKey"],
});
```
...
this.logger.configure({
    transports: [
        ConsoleTransport.forProduction(),  // JSON is faster
    ]
});
```
...
```typescript
this.logger.configure({ level: LogLevel.WARN });
```

3. **Limit buffer size:**
```typescript
this.logger.configure({
    query: { bufferSize: 100 }  // Smaller buffer
});
```
