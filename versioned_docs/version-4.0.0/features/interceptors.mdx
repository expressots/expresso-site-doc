...
Each interceptor's `next.handle()` resolves the next item in the pipe. Short-circuit by returning early — the rest of the pipe is skipped.
...
| `PerformanceInterceptor`     | Per-request timing — feeds the Studio "slow routes" view.                                |
