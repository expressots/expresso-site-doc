...
Interceptors provide a way to add cross-cutting concerns to your application without modifying controller code. They follow the **Aspect-Oriented Programming (AOP)** pattern, allowing you to intercept requests and responses to add functionality like logging, caching, transformation, timing, and more.

## Overview

Interceptors in ExpressoTS provide:

- **Pre/Post Processing**: Execute code before and after route handlers
- **Priority-Based Execution**: Control interceptor execution order
- **Dependency Injection**: Full DI support for interceptors
- **One-Liner Setup**: Simple setup with `setupInterceptorsForExpress()`
- **Built-In Interceptors**: Performance tracking out of the box
...
Interceptors are automatically applied to all routes. You can also apply specific interceptors to controllers or methods:

```typescript
import { controller, Get, useInterceptor } from "@expressots/adapter-express";

@controller("/users")
@useInterceptor(CacheInterceptor)  // Apply to all routes in controller
export class UserController {
    
    @Get("/:id")
    @useInterceptor(LoggingInterceptor)  // Apply to specific route
    getUser() {
...
2. Have `@Interceptor()` decorator
3. Have `@provide()` decorator for DI
4. Define a `priority` property
5. Implement `intercept()` method

```typescript
import { provide, Interceptor } from "@expressots/core";
import type { IInterceptor, ExecutionContext, CallHandler } from "@expressots/core";
...
Registered via `setupInterceptorsForExpress()`, automatically applied to all routes:
...
```typescript
@Get("/data")
@useInterceptor(AuthInterceptor)      // Priority: 1
@useInterceptor(LoggingInterceptor)   // Priority: 5
@useInterceptor(CacheInterceptor)     // Priority: 10
getData() {
    return { data: "example" };
}
```
...
@provide(MyInterceptor)         // Required
```
