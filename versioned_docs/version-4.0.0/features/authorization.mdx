...
```mermaid
flowchart LR
    Req[Request arrives] --> Extract[ScopeExtractor]
    Extract --> Auth[AuthProvider.authenticate]
    Auth --> Preload{enablePreloading?}
    Preload -->|yes| Resolve[PermissionPreloader resolves all permissions once]
    Preload -->|no| OnDemand[Guards resolve permissions when first read]
    Resolve --> Cache[Per-request cache]
    OnDemand --> Cache
    Cache --> Guard[Guard executes]
    Guard --> Allow[Allow / 401 / 403]
```
