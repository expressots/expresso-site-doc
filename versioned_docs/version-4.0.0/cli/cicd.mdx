...
| `--docker-registry`     | `-r`  | —                | Image registry URL for the publish stage (e.g. `ghcr.io/acme`).             |
...
`cicd` uses the [`expressots/templates`](https://github.com/expressots/templates) repository on its `main` branch. Customize them with [`expressots templates repo set <url>`](./templates.mdx#custom-template-repository).
