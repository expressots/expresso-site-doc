...
| ------- | ----- | ---------------------------------------------------- | ---- |
| `new`   | —     | Create a new project from a template (`application` / `micro`) | [new](./new.mdx) |
| `generate` | `g` | Scaffold a single resource (controller, use case, DTO, interceptor, event handler, guard, etc.) | [generate](./generate.mdx) |
| `create` | —    | Scaffold a standalone provider package skeleton      | [providers](./providers.mdx#create) |

### Running

| Command | Alias  | Purpose                                                  | Page |
| ------- | ------ | -------------------------------------------------------- | ---- |
| `dev`   | —      | Local dev server with `tsx --watch`. Optional `--container` flag for the Docker flow. | [running](./running.mdx#dev) |
| `build` | —      | Compile to JavaScript with `tsc`. Rewrites alias imports in opinionated projects. | [running](./running.mdx#build) |
| `prod`  | —      | Run the compiled output with `node`                      | [running](./running.mdx#prod) |
...
| `--help`    | —     | yargs-generated help; works on every command (`expressots <cmd> --help`) | — |
