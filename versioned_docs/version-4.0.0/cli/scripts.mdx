...
Unknown names are reported and skipped — the rest still run.
...
`expressots scripts` requires a lockfile to choose a runner. If none is present, install dependencies first (`npm install`, `yarn`, or `pnpm install`) — that creates the lockfile.
:::
