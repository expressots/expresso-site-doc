...
```bash
expressots new my-app --template opinionated
```

## Migration from v4

If you're migrating from the old `createMicroAPI()` API, here are the key changes:

| Before (v4) | After (v5) |
|-------------|------------|
| `createMicroAPI()` | `micro()` |
| `microAPI.build()` | Not needed |
| `app.Middleware.parse()` | Auto-enabled |
| `app.Route.get(path, handler, ...middleware)` | `app.get(path, ...middleware, handler)` |
| `microAPI.Container.addSingleton()` | Use full framework for DI |
