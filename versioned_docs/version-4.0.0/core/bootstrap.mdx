...
| -------------------- | ------------------------------------------------------ | --------------------------- |
| 🔰 Just starting out | [Basic Usage](#basic-usage)                            | Zero config, just works     |
...
| 🤖 CI/CD Pipeline    | [CI/CD Setup](#cicd-support)                           | Auto-detection, zero config |
...
Bootstrap automatically detects CI/CD environments and adjusts behavior for **zero-configuration deployments**.

### Automatic Detection

Bootstrap detects CI/CD by checking platform-specific environment variables:

```typescript
// Generic CI flag (most platforms set this)
process.env.CI = "true";

// Platform-specific detection (9 platforms supported)
...
✅ Buildkite • ✅ Azure DevOps • ✅ Bamboo • ✅ TeamCity
:::
...
When CI/CD is detected, bootstrap automatically:

| Feature                | Local Development          | CI/CD Environment                     |
| ---------------------- | -------------------------- | ------------------------------------- |
| **.env file loading**  | ✅ Loads from disk         | 🚫 Skipped (uses `process.env`)       |
| **Template creation**  | ✅ Creates `.env.template` | 🚫 Never creates files                |
| **Validation**         | ⚠️ Warns on missing        | ❌ **Crashes on missing**             |
| **Error messages**     | Generic                    | 🔧 **Platform-specific hints**        |
| **.env.vault support** | Optional                   | ✅ Auto-loads if `DOTENV_KEY` present |
...
// 2. Warns if DATABASE_URL missing
// 3. App starts anyway
...
// 1. Skips .env file loading
// 2. Checks process.env.DATABASE_URL
// 3. Crashes if missing with platform-specific setup instructions
```
...
When validation fails, you get **actionable setup instructions** for your platform:
...
🔧 GitHub Actions Setup:
   ...
   4. Add secrets for: DATABASE_URL, JWT_SECRET
   5. Use in workflow:
...
🔧 GitLab CI Setup:
   ...
   4. Add variables for: DATABASE_URL, JWT_SECRET
   5. Set "Protect variable" and "Mask variable" as needed
   6. Variables are automatically available in .gitlab-ci.yml
...
🔧 Jenkins Setup:
   ...
   4. Add credentials for: DATABASE_URL, JWT_SECRET
   5. Use in pipeline:
...
📚 Docs: https://www.jenkins.io/doc/book/using/using-credentials/
```

</details>

:::tip .env.vault Support
If you use [dotenv-vault](https://www.dotenv.org/), just set `DOTENV_KEY` in your CI:
...
-   ✅ Detects encrypted `.env.vault` file
-   ✅ Decrypts using `DOTENV_KEY`
...
### Testing CI Detection Locally

Test how your app behaves in CI environments:
...
```bash
[ExpressoTS] 01/09/2026 02:15:43 PM [PID:18484] INFO  [bootstrap] 🔍 CI environment detected: GitHub Actions
[ExpressoTS] 01/09/2026 02:15:43 PM [PID:18484] INFO  [bootstrap] ✅ Skipping .env file loading (using process.env)
[ExpressoTS] 01/09/2026 02:15:43 PM [PID:18484] INFO  [bootstrap] 🔐 Validating required environment variables...
```

:::note Platform Detection

-   `CI=true` (generic) → Shows "CI Platform"
-   `GITHUB_ACTIONS=true` → Shows "GitHub Actions"
-   `GITLAB_CI=true` → Shows "GitLab CI"
-   etc.
    :::
...
| --------------------------- | --------------- | ------------------- |
| **CI Detection**            | ~1-2ms (cached) | ⚡ Negligible       |
| **Platform Identification** | O(1) Map lookup | ⚡ Negligible       |
| **File I/O Saved**          | ~3-8ms          | ⚡ Faster startup   |
| **Total Overhead**          | &lt;1ms         | ⚡ Essentially free |

**Why it matters:**

```typescript
// ❌ Without CI detection
// Startup: ~25ms (includes file I/O)

// ✅ With CI detection
// Startup: ~15ms (skips file I/O)
// Saves: ~10ms per deployment
```
...
ExpressoTS provides seamless Docker development with **one command**.
...
1. ✅ Generates Docker files if missing
2. ✅ Sets up local dependencies (monorepo support)
3. ✅ Starts container with hot reload
4. ✅ Exposes debug port (9229)
5. ✅ Syncs your code via volumes

:::tip Zero Configuration
If Docker files don't exist, they're auto-generated with sensible defaults. Just run the command!
:::
...
# 🔨 Rebuild before starting (after dependency changes)
...
# 🎛️ Combine options
expressots dev -c -b -d  # Rebuild, then run in background
```
...
| ----------------------- | -------------------------------------- | ------------------------------ |
| 🔄 **Hot Reload**       | File changes restart app automatically | No manual restarts             |
| 🔍 **Debug Port**       | Node.js inspector on port 9229         | Attach VS Code debugger        |
| 📦 **Volume Mounts**    | Code synced between host ↔ container   | Edit locally, run in container |
| 🗄️ **Auto Services**    | Redis, PostgreSQL if detected in deps  | Full stack in one command      |
| 📊 **Health Checks**    | Container health monitoring            | Know when app is ready         |
| 🏗️ **Monorepo Support** | Handles `file:../` dependencies        | Works with complex setups      |

---

### Debugging in Containers

Attach VS Code debugger to your containerized app:

**1. Start container with debug:**

```bash
expressots dev --container
# Debug port 9229 is automatically exposed
```
...
:::tip Hot Reload + Debugging
Both work simultaneously:

-   ✅ Change file → App restarts
-   ✅ Set breakpoint → Debugger reconnects
-   ✅ Full debugging experience in container
    :::
...
For more control, use `containerize` command:
...
| Preset          | Files Generated                      | Use Case                  |
| --------------- | ------------------------------------ | ------------------------- |
| 🎯 **Minimal**  | `Dockerfile` only                    | Simple apps, manual setup |
| 📦 **Standard** | Dockerfile + Compose + .dockerignore | Most projects             |
| 🚀 **Full**     | Everything + K8s + CI/CD             | Production-ready          |

<details>
<summary>📁 <strong>What files are generated?</strong></summary>
...
docker-setup.js                 # Local dependency setup
k8s/
...
### Local Dependencies (Monorepo Support)

If your project uses `file:../` dependencies, ExpressoTS handles it automatically:

```json
// package.json
{
    "dependencies": {
        "@myorg/shared": "file:../shared", // ← Monorepo dependency
...
1. ✅ Creates `.docker-deps/` with packed dependencies
2. ✅ Generates `package.docker.json` with fixed paths
3. ✅ Updates `Dockerfile.development` to use packed deps
4. ✅ Adds setup script to `package.json`
...
:::warning Important
Run `npm run docker:setup` whenever you:

-   Add new local dependencies
-   Update local dependency versions
-   Switch branches with different local deps
    :::
...
⚠️  Warning: Image size is 485MB (consider alpine base)
⚠️  Warning: 12 layers (consider combining RUN commands)
💡 Optimization: Use .dockerignore (potential 120MB savings)

📊 Recommendations:
   1. Switch to node:20-alpine (saves ~200MB)
   2. Combine RUN commands (reduces layers by 8)
...
Generate CI/CD pipelines that build and deploy your containerized app:
...
<details>
<summary>📋 <strong>Example: GitHub Actions workflow</strong></summary>
...
ExpressoTS CLI v4 uses a **dynamic template system** - templates stay up-to-date without CLI updates.

### How It Works

```
┌──────────────────────────────────────────────────────┐
│ 1. Remote Templates (GitHub)                        │
│    └─ expressots/templates repo                     │
...
| Feature                 | Description                                    |
| ----------------------- | ---------------------------------------------- |
| 🔄 **Always Current**   | Get latest best practices without updating CLI |
| 🌐 **Community-Driven** | Anyone can contribute improvements             |
| 🏢 **Customizable**     | Use your org's templates                       |
| 📴 **Offline Support**  | Works without internet                         |
| 📦 **Version Control**  | Templates versioned independently from CLI     |
...
# Output:
# Available templates:
#   - docker/minimal (v2.1.0)
#   - docker/standard (v2.1.0)
#   - docker/full (v2.1.0)
#   - cicd/github (v1.5.0)
#   - cicd/gitlab (v1.5.0)
#   ...
...
# Output:
# ✅ Downloaded 15 templates (cached for 24h)
...
#   Location: ~/.expressots/cache/templates/
#   Last updated: 2 hours ago
#   Templates: 15
#   Cache expires in: 22 hours
...
Anyone can contribute to the template system:

**What you can contribute:**

-   🐳 **Docker configurations**  
    Optimized Dockerfiles, multi-stage builds, security best practices

-   🤖 **CI/CD strategies**  
    GitHub Actions, GitLab CI, CircleCI workflows

-   ☸️ **Kubernetes manifests**  
    Deployments, services, ingress configs, helm charts

-   💰 **Pricing data**  
    Cloud platform pricing, cost optimizations

-   📚 **Documentation**  
    Template usage guides, examples

**How to contribute:**

1. Fork `expressots/templates` repository
2. Add/update templates following structure:
    ```
    templates/
    ├── docker/
    │   └── my-template/
...
4. After merge, templates available to all users!

:::tip Quick PR Turnaround
Template changes don't require CLI releases:

-   ✅ Your PR merged → Available immediately
-   ✅ Users run `expressots templates update`
-   ✅ Everyone gets your improvements!
    :::
...
```bash
# Set your org's template repo
expressots templates repo set mycompany/expressots-templates

# Now all generations use your templates
expressots containerize docker
# Uses: github.com/mycompany/expressots-templates/docker/*
```

**Example use cases:**

-   🏢 Company-specific Docker configurations
-   🔒 Security-hardened templates
-   🎨 Branded startup banners
-   ⚙️ Pre-configured CI/CD workflows
...
Templates are versioned independently from CLI:
...
# Template version
expressots templates list
# docker/standard (v2.1.0)  ← Template version
# cicd/github (v1.5.0)
```
...
```typescript
// ❌ Old way: Templates tied to CLI version
// - Update Docker best practices → Must release new CLI
// - Users must update CLI to get new templates
// - Slow iteration

// ✅ New way: Independent versioning
// - Update Docker template → Just update template repo
// - Users run `expressots templates update`
// - Fast iteration, no CLI update needed
```
...
CLI works without internet using embedded templates:
...
# Output:
# ⚠️  Cannot fetch remote templates (offline)
# ✅ Using embedded templates (v2.0.0)
...
| Scenario                      | Behavior                      |
| ----------------------------- | ----------------------------- |
| 🌐 **Online + Fresh cache**   | Uses cached templates (fast)  |
| 🌐 **Online + Expired cache** | Fetches latest, updates cache |
| 📴 **Offline + Valid cache**  | Uses cached templates         |
| 📴 **Offline + No cache**     | Uses embedded templates       |

---

### Learn More

-   📚 [Templates & Customization](../cli/templates.mdx) - Detailed template guide
-   💰 [Pricing Data Management](../cli/pricing.mdx) - Cloud cost data
-   🤝 [Contributing to Templates](../cli/contributing.mdx) - Contribution guide
...
See [CI/CD Support](#cicd-support) for details.
